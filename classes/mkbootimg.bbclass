...
do_deploy:append() {
	...
	mkdtboimg create ${MKBOOTIMG_DT_ARGS} "${B}"/dt.dtb.android "${DTB_OUTPUT}"
        ...
        if [ "${MKBOOTIMG_VENDOR_BOOT}" = "true" ]; then
            ...
            deploy_bootimg "${DISTRO}-${MACHINE}-vendor_boot.img"
        fi

        # Not all devices with v3/4 boot images have init_boot
        if [ "${MKBOOTIMG_INIT_BOOT}" = "true" ]; then
            # Stub out init_boot.img
            mkbootimg -o "${B}"/"${DISTRO}-${MACHINE}-init_boot.img" \
                      --header_version "${MKBOOTIMG_HEADER_VERSION}"

            # Deploy init_boot.img
            deploy_bootimg "${DISTRO}-${MACHINE}-init_boot.img"
        fi
    else
    ...
    # Deploy boot.img
    deploy_bootimg "${DISTRO}-${MACHINE}-boot.img"
...
EOF
        ...
        mkdtboimg create ${MKBOOTIMG_DTBO_ARGS} "${B}"/"${DISTRO}-${MACHINE}-dtbo.img" "${B}"/dt.dtbo

        # Deploy the DTBO
        deploy_bootimg "${DISTRO}-${MACHINE}-dtbo.img"
    fi
}

FILES:${KERNEL_PACKAGE_NAME}-image += "/${KERNEL_IMAGEDEST}/${DISTRO}-${MACHINE}-boot.img"
FILES:${KERNEL_PACKAGE_NAME}-image += "${@'/' + d.getVar('KERNEL_IMAGEDEST') + '/' + d.getVar('DISTRO') + '-' + d.getVar('MACHINE') + '-vendor_boot.img' if d.getVar('MKBOOTIMG_VENDOR_BOOT') == 'true' else ''}"
FILES:${KERNEL_PACKAGE_NAME}-image += "${@'/' + d.getVar('KERNEL_IMAGEDEST') + '/' + d.getVar('DISTRO') + '-' + d.getVar('MACHINE') + '-init_boot.img' if d.getVar('MKBOOTIMG_INIT_BOOT') == 'true' else ''}"
FILES:${KERNEL_PACKAGE_NAME}-image += "${@'/' + d.getVar('KERNEL_IMAGEDEST') + '/' + d.getVar('DISTRO') + '-' + d.getVar('MACHINE') + '-dtbo.img' if d.getVar('MKBOOTIMG_DTBO') == 'true' else ''}"
